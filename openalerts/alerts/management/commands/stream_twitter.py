    ...
    def handle(self, *args, **options):
        ...
        for source in sources:
            if source.identifier == None:
                self.stdout.write(
                    f"Populating ID for new source {source.human_readable_identifier}..."
                )
        ...
        class OpenAlertsStreamListener(tweepy.StreamListener):
            def on_status(self, status):
                ...
                source = Source.objects.filter(
                    source="TWITTER", identifier=status.user.id_str
                )
                if source.exists():
                    source = source.first()
                else:
                ...
                Alert.objects.create(
                    ...
                    url=url,
                    source=source,
                    source_identifier=status.id_str,
                )

                cmd.stdout.write(
                    f"Published new alert from {source.human_readable_identifier} in channel {source.channel}: '{text}'"
                )
        ...
        self.stdout.write(
            self.style.SUCCESS(
                f"Opening stream monitoring {sources.count()} sources on Twitter as @{user.screen_name}..."
            )
        )
